"""
Tests the meanColumnValue constraint.

The implementation should reject all rows in a column if the column mean value is not between the expected values.
"""
import pytest
...
def test_should_return_df_without_changes_if_empty_df_with_mean_constraint(spark_session):
...
def test_should_return_df_without_changes_if_the_mean_is_between_given_values(spark_session):
...
def test_should_reject_all_rows_if_mean_is_smaller_than_given_values(spark_session):
    df = spark_session.createDataFrame([[5], [10], [15]], schema=single_integer_column_schema)
...
def test_should_reject_all_rows_if_mean_is_larger_than_given_values(spark_session):
    df = spark_session.createDataFrame([[5], [10], [15]], schema=single_integer_column_schema)
...
def test_mean_should_check_all_given_columns_separately(spark_session):
    df = spark_session.createDataFrame([[5, 1], [10, 2], [15, 3]], schema=two_integer_columns_schema)
    expected_errors = spark_session.createDataFrame([], schema=two_integer_columns_schema)
